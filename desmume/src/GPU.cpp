...
{
	IOREG_EXT_SETTINGS *EXT_SETTINGS = &this->_IORegisterMap->EXT_SETTINGS;
	u16 lineWidth = (COMPOSITORMODE == GPUCompositorMode_Debug) ? compInfo.renderState.selectedBGLayer->size.width : GPU_FRAMEBUFFER_NATIVE_WIDTH;
	u16 orig_line_width = lineWidth;
...
#endif
	if (!(EXT_SETTINGS->UncenterLayer & (1 << compInfo.renderState.selectedBGLayer->layerID)))
	{
		x.value -= (dx * (GPU_FRAMEBUFFER_NATIVE_WIDTH - 256) / 2);
		y.value -= (dy * (GPU_FRAMEBUFFER_NATIVE_WIDTH - 256) / 2);
	}
	...
	if (dx == 256 && dy == 0)
	{
		...
		if ( WRAP || ((auxX >= 0) && (auxX + orig_line_width <= wh) && (auxY >= 0) && (auxY < ht)) )
		{
...
void GPUEngineBase::_RenderLine_BGText(GPUEngineCompositorInfo &compInfo, const u16 XBG, const u16 YBG)
{
	...
	size_t pixCountLo = 8 - (XBG & 0x0007);
	size_t x = 0;
	size_t xoff = XBG;
	u16 lineWidth = (COMPOSITORMODE == GPUCompositorMode_Debug) ? compInfo.renderState.selectedBGLayer->size.width : GPU_FRAMEBUFFER_NATIVE_WIDTH;
	...
	if (!(EXT_SETTINGS->DisableAutoClip & (1 << compInfo.renderState.selectedBGLayer->layerID)))
	{
		if (lg == 256)
		{
			x = (GPU_FRAMEBUFFER_NATIVE_WIDTH - 256) / 2;
			pixCountLo += x;
			lineWidth = (256 + x);
		}
		else
		{
			if (!(EXT_SETTINGS->UncenterLayer & (1 << compInfo.renderState.selectedBGLayer->layerID)))
			{
				xoff -= (GPU_FRAMEBUFFER_NATIVE_WIDTH - 256) / 2;
			}
		}
	}
	else
		...
		case BGType_AffineExt_Direct: // direct colors / BMP
		{
			...
			if (!MOSAIC)
			{
				...
				if (!isRotationScaled)
				{
					...
					if (vramPixel < (256 * GPU_VRAM_BLOCK_LINES * 4))
					{
...
bool GPUEngineBase::_ComputeSpriteVars(GPUEngineCompositorInfo &compInfo, const OAMAttributes &spriteInfo, SpriteSize &sprSize, s32 &sprX, s32 &sprY, s32 &x, s32 &y, s32 &lg, s32 &xdir)
{
	...
	lg = sprSize.width;
	int line_width = GPU_FRAMEBUFFER_NATIVE_WIDTH;
	int x_ofs = 0;
	int min_x = 0;
...
// and how a step to the right in a screenline translates within the sprite
	...
	if (!EXT_SETTINGS->LeftAlignSprites)
	{
		sprX += (GPU_FRAMEBUFFER_NATIVE_WIDTH - 256) / 2;
	}
	if (sprX < -128)
	{
		sprX += 512;
...
void GPUEngineBase::_SpriteRenderPerform(GPUEngineCompositorInfo &compInfo, u16 *__restrict dst, u8 *__restrict dst_alpha, u8 *__restrict typeTab, u8 *__restrict prioTab)
{
	...
	for (size_t spriteNum = 0; spriteNum < 128; spriteNum++)
	{
		...
		if (spriteInfo.RotScale != 0)
		{
			...
			if (!EXT_SETTINGS->LeftAlignSprites)
			{
				frameX += (GPU_FRAMEBUFFER_NATIVE_WIDTH - 256) / 2;
			}
			if (frameX < -128)
			{
				frameX += 512;
...
void GPUEngineBase::_RenderLine_LayerOBJ(GPUEngineCompositorInfo &compInfo, itemsForPriority_t *__restrict item)
{
	...
	if (item->nbPixelsX == 256 && useCustomVRAM)
	{
		if (this->_isLineRenderNative[compInfo.line.indexNative])
		{
...
void GPUEngineBase::_UpdateWINH(GPUEngineCompositorInfo &compInfo)
{
	...
	size_t windowLeft  = (WIN_NUM == 0) ? this->_IORegisterMap->WIN0H.Left  : this->_IORegisterMap->WIN1H.Left;
	size_t windowRight = (WIN_NUM == 0) ? this->_IORegisterMap->WIN0H.Right : this->_IORegisterMap->WIN1H.Right;

	if (EXT_SETTINGS->StretchWindow)
	{
		float window_temp_rounded_l = ((float)windowLeft * GPU_FRAMEBUFFER_NATIVE_WIDTH / 256) + 0.5;
		float window_temp_rounded_r = ((float)windowRight * GPU_FRAMEBUFFER_NATIVE_WIDTH / 256) + 0.5;
		windowLeft = (size_t)window_temp_rounded_l;
		windowRight = (size_t)window_temp_rounded_r;
	...
	{
		size_t window_ofs = (GPU_FRAMEBUFFER_NATIVE_WIDTH - 256) / 2;
		windowLeft += window_ofs;
		windowRight += window_ofs;

		if (windowLeft > windowRight)
		{
			memset(&this->_h_win[WIN_NUM][window_ofs], 1, 256 * sizeof(u8));
			memset(&this->_h_win[WIN_NUM][windowRight+1], 0, (windowLeft - (windowRight + 1)) * sizeof(u8));
		}
		else
		{
			memset(&this->_h_win[WIN_NUM][window_ofs], 0, 256 * sizeof(u8));
			memset(&this->_h_win[WIN_NUM][windowLeft], 1, (windowRight - windowLeft) * sizeof(u8));
		}
		memset(&this->_h_win[WIN_NUM][0], this->_h_win[WIN_NUM][window_ofs], window_ofs * sizeof(u8));
		memset(&this->_h_win[WIN_NUM][window_ofs+256], this->_h_win[WIN_NUM][window_ofs+255], window_ofs * sizeof(u8));
	}
}
...
GPUEngineA::GPUEngineA()
{
	...
	_VRAMNativeBlockPtr[1] = _VRAMNativeBlockPtr[0] + (1 * GPU_VRAM_BLOCK_LINES * 256);
	_VRAMNativeBlockPtr[2] = _VRAMNativeBlockPtr[0] + (2 * GPU_VRAM_BLOCK_LINES * 256);
	_VRAMNativeBlockPtr[3] = _VRAMNativeBlockPtr[0] + (3 * GPU_VRAM_BLOCK_LINES * 256);
	
	memset(this->_VRAMNativeBlockCaptureCopy, 0, GPU_VRAM_BLOCK_LINES * 256 * 4);
	_VRAMNativeBlockCaptureCopyPtr[0] = this->_VRAMNativeBlockCaptureCopy;
	_VRAMNativeBlockCaptureCopyPtr[1] = _VRAMNativeBlockCaptureCopyPtr[0] + (1 * GPU_VRAM_BLOCK_LINES * 256);
	_VRAMNativeBlockCaptureCopyPtr[2] = _VRAMNativeBlockCaptureCopyPtr[0] + (2 * GPU_VRAM_BLOCK_LINES * 256);
	_VRAMNativeBlockCaptureCopyPtr[3] = _VRAMNativeBlockCaptureCopyPtr[0] + (3 * GPU_VRAM_BLOCK_LINES * 256);
	...
	else
	{
		for (size_t line = 0; line < compInfo.line.renderCount; line++)
		{
			for (compInfo.target.xCustom = 0; compInfo.target.xCustom < compInfo.line.widthCustom; compInfo.target.xCustom++, compInfo.target.lineColor16++, compInfo.target.lineColor32++, compInfo.target.lineLayerID++)
			{
				...
				int bg_width = (compInfo.line.widthCustom * 256 * 2) / GPU_FRAMEBUFFER_NATIVE_WIDTH;
				if (srcX >= bg_width)
				{
					srcX -= bg_width;
				}
...
void GPUEngineA::_RenderLine_DisplayCapture(const GPUEngineCompositorInfo &compInfo)
{
	...
	const bool isRead3DLineNative = false;
	...
	size_t dstNativeOffset = (DISPCAPCNT.VRAMWriteOffset * 64 * 256) + (compInfo.line.indexNative * CAPTURELENGTH);
...
GPUSubsystem::GPUSubsystem()
{
	...
	_displayInfo.isCustomSizeRequested = true;
...
void GPUSubsystem::SetCustomFramebufferSize(size_t w, size_t h)
{
	...
	this->_displayInfo.isCustomSizeRequested = ( (w != 256) || (h != GPU_FRAMEBUFFER_NATIVE_HEIGHT) );
