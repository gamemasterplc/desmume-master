...
#define GPU_FRAMEBUFFER_NATIVE_WIDTH	320
...
typedef union
{
	...
	{
#ifndef MSB_FIRST
		u16 DisableAutoClip : 4;				//   0-3: Clip Non-Scrolled 256x256 or 256x512 Backgrounds; 0=Disable, 1=Enable
		u16 UncenterLayer: 4;					//   4-7: Left-Align Backgrounds; 0=Disable, 1=Enable
		u16 LeftAlignSprites : 1;				//   8-9: Left Align Sprites; 0=Disable, 1=Enable
		u16 StretchWindow : 1;					//   10: Stretch Windows to 16:9
		u16 : 6;								//	 11-15: Unused bits
#else
		u16 UncenterLayer: 4;					//   4-7: Left-Align Backgrounds; 0=Disable, 1=Enable
		u16 DisableAutoClip : 4;				//   0-3: Clip Non-Scrolled 256x256 or 256x512 Background; 0=Disable, 1=Enable
...
} IOREG_EXT_SETTINGS;							// 0x400x058: Extended Settings
...
typedef struct
{
	...
	u8						unused[2];				// 0x0400x056
	IOREG_EXT_SETTINGS		EXT_SETTINGS;			// 0x0400x058
	u8						unused_2[6];			// 0x0400x05A
...
typedef struct
{
	u16 PixelsX[GPU_FRAMEBUFFER_NATIVE_WIDTH];
	u8 BGs[NB_BG], nbBGs;
	u8 pad[1];
	u16 nbPixelsX;
	//256+8:
	u8 pad2[512 - GPU_FRAMEBUFFER_NATIVE_WIDTH - 8];
...
protected:
	...
	{
		CACHE_ALIGN MosaicTableEntry table[16][GPU_FRAMEBUFFER_NATIVE_WIDTH];
	...
	struct MosaicColor {
		u16 bg[4][GPU_FRAMEBUFFER_NATIVE_WIDTH];
		struct Obj {
			u16 color;
			u8 alpha;
			u8 opaque;
	...
	CACHE_ALIGN u8 _sprNum[GPU_FRAMEBUFFER_NATIVE_WIDTH];
