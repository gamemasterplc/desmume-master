...
void EditHud(s32 x, s32 y, HudStruct *hudstruct) {
	...
	while (!IsHudDummy(&hudstruct->hud(i))) {
		...
		if(hud.x < 0)  hud.x = 0;
		if(hud.y < 0)  hud.y = 0;
		if(hud.x > 309)hud.x = 309; //margins
...
static void OSD_HandleTouchDisplay() {
	...
	temptouch.X = ((NDS_getRawUserInput().touch.touchX >> 4)*320)/256;
	...
	if(touchshadow) {
		...
		for (int i = 0; i < 8; i++) {
			...
			if(temptouch.X != 0 || temptouch.Y != 0) {
				aggDraw.hud->lineColor(0, 319, 0, touchalpha[i]);
				aggDraw.hud->line(temptouch.X - 320, temptouch.Y + 192, temptouch.X + 320, temptouch.Y + 192); //horiz
				aggDraw.hud->line(temptouch.X, temptouch.Y - 320, temptouch.X, temptouch.Y + 384); //vert
		...
		if(NDS_getRawUserInput().touch.isTouch) {
			aggDraw.hud->lineColor(0, 319, 0, 128);
			aggDraw.hud->line(temptouch.X - 320, temptouch.Y + 192, temptouch.X + 320, temptouch.Y + 192); //horiz
			aggDraw.hud->line(temptouch.X, temptouch.Y - 320, temptouch.X, temptouch.Y + 384); //vert
		}
	...
	if(nds.isTouch)
	{
		temptouch.X = ((nds.scr_touchX / 16)*320)/256;
		temptouch.Y = nds.scr_touchY / 16;
		aggDraw.hud->lineColor(255, 0, 0, 128);
		aggDraw.hud->line(temptouch.X - 320, temptouch.Y + 192, temptouch.X + 256, temptouch.Y + 192); //horiz
		aggDraw.hud->line(temptouch.X, temptouch.Y - 320, temptouch.X, temptouch.Y + 384); //vert
	}
